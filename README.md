##Intro
Ice is a AWS usage tool. Ice provides a birds-eye view of our large and complex cloud landscape.  Cloud resources are dynamically provisioned by tens of service teams within the organization, any static snapshot constructed of resource allocation has limited value.  The ability to trend usage patterns on a global scale, yet decompose down to a region, availability zone, or service team provides incredible flexibility. Ice allows us to quantify our AWS footprint and make educated decisions regarding purchases and reallocation of specific resources.
...
##What it does
Ice communicates with AWS biling APIs and maintains knowledge of the following key AWS entity categories:
- Accounts
- Regions
...
In addition Ice allows the definition of Application Groups. These groups are explicitly defined collections of Applications in your organization. Such groups allows usage and cost information to be aggregated by individual engineering service teams within your organization, of which each might deploy multiple services.

When representing the cost profile for individual resources Ice will factor the depreciation schedule into your cost contour if so desired.  The ability to amortize one-time purchase costs over time allows teams to better evaluate their month-to-month cost footprint.
...
5. Ice uses [highstock](http://www.highcharts.com/) to generate interactive graphs. Please make sure you purchase the proper license before using it.
...
1. Find the s3 billing bucket name and billing file prefix and specify them in ice.properties. For example:
  ...
  Tip: If Ice is running from a different account of the s3 billing bucket, for example Ice is running in account "test", while the billing files are written to bucket in account "prod", sometimes account "test" might not be able to read those billing files, because Amazon wrote those billing files. In this case, you can create a secondary s3 bucket in account "prod" and grant read access to account "test", and then create a billing file poller running in account "prod" to copy billing files to the secondary bucket.
...
2. Processor configuration
  ...
  2.5 In ice.properties, specify the start time in millis the processor will start processing billing files. For example, if you want to start processing billing files from April 1, 2013:
...
1. Basic reservation service
  ...
  If you have reserved instances in your accounts, you may want to make use of the reservation view in UI, where you can browse/analyze your on-demand, un-used reserved instance usage&cost of different instance types in different regions, zones and accounts. in Bootstrap.groovy, BasiicReservationService is used. You can specify reservation period and reservation utilization type in ice.properties:
...
3. On-demand instance cost alert
  ...
  You can set set an on-demand instance cost threshold, alert emails will be sent through Amazon SES if the threshold is reached within last 24 hours. The alert emails will be sent no more than once a day. The following properties are needed in ice.properties:
...
4. Sharing reserved instances among accounts (*)
  ...
  If different accounts have different AZ mappings, you also need to subclass BasicAccountService and override method getAccountMappedZone.
   
5. Customized reservation service (*)

  Reserved instance prices in BasiicReservationService are copied from Amazon's ec2 price page as of Jun 1, 2013. Likely, your accounts may have different reservation prices (e.g. Amazon will change prices in the future, or you may negotiate special prices with Amazon). In this case, you need to write a subclass of BasicReservationService to provide the correct pricing.
...
8. Throughput metric service (*)
  ...
  You may also want to show your organization's throughput metric with the aws usage and cost. You can choose to implement interface ThroughputMetricService, or you can simply use the existing BasicThroughputMetricService. Using BasicThroughputMetricService requires the throughput metric data is store monthly in files with names like <filePrefix>_2013_04, <filePrefix>_2013_05. Data in file are delimited by new lines. <filePrefix> is specified when you create BasicThroughputMetricService instance.
