    ...
    <table>
      ...
      <tr ng-repeat="legend in legends | filter:filter_legend" class="{{getTrClass($index)}}"  style="{{legend.style}};">
        ...
        <td ng-repeat="header in headers" ng-switch on="legend.name">
          <a ng-switch-when="aggregated" ng-href="detail#groupBy={{legendName}}&start={{header.start}}&end={{header.end}}">$ {{data[legend.name][header.index] | number:2}}</a>
          <a ng-switch-default ng-href="detail#{{legendName.toLowerCase()}}={{legend.name}}&groupBy={{nextGroupBy(legendName)}}&start={{header.start}}&end={{header.end}}">$ {{data[legend.name][header.index] | number:2}}</a>
        </td>
      </tr>
      </tbody>
    </table>
